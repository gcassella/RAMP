...
class SIsotropic(SPrim):
    ...
    def scatter_prg(self, queue, N, neutron_buf, intersection_buf, iidx_buf):
        self.prg.isotropic_scatter(queue, (N,),
                                   ...
                                   np.float64(self.sigma_scat)).wait()

    def _LoadSQW(self, fn):
        with open(fn, 'r') as fin:
            lines = fin.readlines()
            q = np.fromstring(lines[17], dtype=np.float64, sep=' ')
            w = np.fromstring(lines[20], dtype=np.float64, sep=' ')
            rho = np.float64(re.findall(r"[-+]?\d*\.\d+|\d+", lines[6])[0])
            sigma_abs = np.float64(re.findall(r"[-+]?\d*\.\d+|\d+", lines[9])[0])
            sigma_coh = np.float64(re.findall(r"[-+]?\d*\.\d+|\d+", lines[10])[0])

        sqw = np.loadtxt(fn, skiprows=23).astype(np.float64) + np.finfo(float).eps.astype(np.float64)
        sigma_scat = sigma_coh

        pw = simps(sqw.T, axis=1, x=q) / np.linalg.norm(sqw)
        pw_cdf = np.array([simps(pw[:i], x=w[:i]) for i in range(1,len(w)+1)], dtype=np.float64)
        ...
        pq_cdf = np.array(
            ...
            for i in range(1,len(q)+1)] for j in range(0,len(w))], dtype=np.float64)
